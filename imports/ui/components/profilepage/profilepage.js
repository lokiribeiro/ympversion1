      ...
      if (file) {
        ...
        this.uploader.send(file, function (error, downloadUrl) {
          if (error) {
            ...
            var userID = $stateParams.employeeId;
            var selector = {userID: userID};
            var profile = Profiles.findOne(selector);

            Meteor.call('upsertProfilePhoto', profile._id, downloadurl, function (err, result) {
              console.log(downloadUrl);
              console.log('success: ' + downloadUrl);
              if (err) {
                console.info('err', err);
            ...
            Meteor.call('upsertPhotoUser', $stateParams.employeeId, downloadUrl, function (err, result) {
              console.log(downloadUrl);
              console.log('success: ' + downloadUrl);
              if (err) {
                console.info('err', err);
